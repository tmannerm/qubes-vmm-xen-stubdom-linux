...
static void qubesgui_message_handler(void *opaque)
{
	...
	if (!libvchan_is_open(qs->vchan)) {
		qs->init_done = 0;
		qs->init_state = 0;
		qemu_set_fd_handler(libvchan_fd_for_select(qs->vchan), NULL, NULL, NULL);
	...
	write_data(qs->vchan, NULL, 0);	// trigger write of queued data, if any present
	if (libvchan_data_ready(qs->vchan) == 0) {
		return;
	}
...
int qubesgui_pv_display_init(int log_level)
{
	...
	/* FIXME: 0 here is hardcoded remote domain */
	qs->vchan = peer_server_init(0, 6000);
	qemu_set_fd_handler(libvchan_fd_for_select(qs->vchan), qubesgui_message_handler, NULL,
			    qs);
	qubesgui_init_connection(qs);
