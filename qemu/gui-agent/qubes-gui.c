...
void send_pixmap_mfns(QubesGuiState * qs)
{
	...
	n = (4 * surface_width(qs->surface) * surface_height(qs->surface) + offset + (XC_PAGE_SIZE-1)) / XC_PAGE_SIZE;
	...
	fprintf(stderr, "dumping mfns: n=%d, w=%d, h=%d\n", n, surface_width(qs->surface), surface_height(qs->surface));
	for (i = 0; i < n; i++)
		u2mfn_get_mfn_for_page_with_fd(qs->u2mfn_fd, (long) data_aligned + i * XC_PAGE_SIZE, (int *) &mfns[i]);
...
void qubesgui_init_connection(QubesGuiState * qs)
{
	...
	if (qs->init_state == 1) {
		...
		fprintf(stderr,
			...
			__LINE__);
		// If we don't have a surface yet just send an arbitary windows
